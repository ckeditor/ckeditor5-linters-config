...
ruleTester.run( ruleName, rule, {
	valid: [
		...
		{
			name: 'Header followed by a blank line and a rule',
			options,
			code: header + '\n\n' + body + '\n'
		},
		{
			name: 'Empty rule body - header is still validated',
			options,
			code: header + '\n\n.foo {}\n'
		}
	],
	...
	invalid: [
		...
		{
			name: 'First comment carries @license but the wording is wrong - replaced',
			options,
		...
		{
			name: 'First comment has no @license tag - the prior heuristic mistook a later "copyright" comment for the header and ' +
				'replaced it. The new rule treats this as missing and prepends a proper header above',
			options,
			code: '/* Editor styles. */\n\n/* (c) 2026 something */\n\n' + body + '\n',
			errors: [ missingHeaderError ],
			output: header + '\n\n/* Editor styles. */\n\n/* (c) 2026 something */\n\n' + body + '\n'
		...
		{
			name: 'First comment lacks @license and contains the word "copyright" - previously misidentified as the license header',
			options,
		...
		{
			name: 'Whitespace before the header',
			options,
			code: '\n\n' + header + '\n\n' + body + '\n',
		...
		{
			name: 'Missing blank line between header and the next rule',
			options,
			code: header + '\n' + body + '\n',
